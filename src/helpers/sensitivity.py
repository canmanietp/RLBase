    ...
    for s, a, r, ns in sars:  # range(agent.observation_space):
        if s not in state_var_sensitivity:
            ...
            for v in state_variables:
                samples = list(range(ranges[v][0], ranges[v][1] + 1))# np.random.uniform(low=ranges[v][0], high=ranges[v][1]+1, size=num_samples) #
                Qs = agent.Q_table[s]
                diff = 0.
                count_samps = 1
...
def do_sensitivity_analysis_single_state(agent, ranges, s, state_variables, num_samples=20):
    # state variables = [0, 1, 2]
    # history = [[s0, a0, r0, s1], ...]
    ...
    decoded_state = list(agent.env.decode(s))
    for v in state_variables:
        samples = np.random.uniform(low=ranges[v][0], high=ranges[v][1]+1, size=num_samples) # list(range(ranges[v][0], ranges[v][1] + 1))  #
        Qs = agent.Q_table[s]
        diff = 0.
        count_samps = 1
